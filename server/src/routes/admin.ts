...
// ---------------------------------------------------------------------------
// Read-only access — students (lab-admin only, scoped to their lab)
// ---------------------------------------------------------------------------
...
router.get('/students', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await pool.query(`SELECT sp.*, u.first_name, u.last_name, u.email FROM student_profiles sp JOIN users u ON u.id = sp.user_id ${where} ORDER BY sp.updated_at DESC`, params);
  ...
  return res.json(result.rows.map((row) => ({ id: row.id, userId: row.user_id, major: row.major, gpa: row.gpa ? parseFloat(row.gpa) : null, graduationYear: row.graduation_year, skills: row.skills || [], bio: row.bio, resumeUrl: row.resume_url, yearLevel: row.academic_level, interests: row.interests || [], firstName: row.first_name, lastName: row.last_name, email: row.email, profileLinks: parseProfileLinks(row.profile_links) })));
}));

router.get('/students/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await pool.query(`SELECT sp.*, u.first_name, u.last_name, u.email FROM student_profiles sp JOIN users u ON u.id = sp.user_id WHERE sp.id = $1 AND EXISTS (SELECT 1 FROM applications a JOIN research_positions rp ON rp.id = a.position_id JOIN pi_profiles pip ON pip.id = rp.pi_id WHERE a.student_id = sp.id AND pip.lab_admin_id = $2)`, [id, req.userId!]);
  ...
  return res.json({ id: row.id, userId: row.user_id, major: row.major, gpa: row.gpa ? parseFloat(row.gpa) : null, graduationYear: row.graduation_year, skills: row.skills || [], bio: row.bio, resumeUrl: row.resume_url, yearLevel: row.academic_level, interests: row.interests || [], firstName: row.first_name, lastName: row.last_name, email: row.email, profileLinks: parseProfileLinks(row.profile_links) });
}));

router.get('/students/:id/resume', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await pool.query(`SELECT sp.resume_url, u.first_name FROM student_profiles sp JOIN users u ON u.id = sp.user_id WHERE sp.id = $1 AND EXISTS (SELECT 1 FROM applications a JOIN research_positions rp ON rp.id = a.position_id JOIN pi_profiles pip ON pip.id = rp.pi_id WHERE a.student_id = sp.id AND pip.lab_admin_id = $2)`, [id, req.userId!]);
  const row = result.rows[0];
  if (!row) return res.status(404).json({ error: 'Student not found or access denied' });
...
router.get('/applications', asyncHandler(async (req: Request, res: Response) => {
  ...
  let query = `SELECT a.*, sp.major, sp.gpa, sp.skills, sp.bio, sp.resume_url, sp.academic_level, u.id AS student_user_id, u.first_name, u.last_name, u.email, rp.title AS position_title, rp.id AS position_id, pip.lab_name, pip.department FROM applications a JOIN research_positions rp ON rp.id = a.position_id JOIN pi_profiles pip ON pip.id = rp.pi_id JOIN student_profiles sp ON sp.id = a.student_id JOIN users u ON u.id = sp.user_id WHERE pip.lab_admin_id = $1`;
  const params: unknown[] = [req.userId!];
  ...
  return res.json(result.rows.map((row) => ({ id: row.id, positionId: row.position_id, studentId: row.student_id, studentUserId: row.student_user_id, status: row.status, coverLetter: row.personal_statement, personalStatement: row.personal_statement, appliedAt: row.created_at, major: row.major, gpa: row.gpa ? parseFloat(row.gpa) : null, skills: row.skills || [], bio: row.bio, resumeUrl: row.resume_url, yearLevel: row.academic_level, firstName: row.first_name, lastName: row.last_name, email: row.email, positionTitle: row.position_title, labName: row.lab_name, department: row.department, questionAnswers: row.question_answers || {} })));
}));

router.get('/applications/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await pool.query(`SELECT a.*, sp.major, sp.gpa, sp.skills, sp.bio, sp.resume_url, sp.academic_level, u.id AS student_user_id, u.first_name, u.last_name, u.email, rp.title AS position_title, rp.id AS position_id, pip.lab_name, pip.department FROM applications a JOIN research_positions rp ON rp.id = a.position_id JOIN pi_profiles pip ON pip.id = rp.pi_id JOIN student_profiles sp ON sp.id = a.student_id JOIN users u ON u.id = sp.user_id WHERE a.id = $1 AND pip.lab_admin_id = $2`, [id, req.userId!]);
  const row = result.rows[0];
  ...
  return res.json({ id: row.id, positionId: row.position_id, studentId: row.student_id, studentUserId: row.student_user_id, status: row.status, coverLetter: row.personal_statement, personalStatement: row.personal_statement, appliedAt: row.created_at, major: row.major, gpa: row.gpa ? parseFloat(row.gpa) : null, skills: row.skills || [], bio: row.bio, resumeUrl: row.resume_url, yearLevel: row.academic_level, firstName: row.first_name, lastName: row.last_name, email: row.email, positionTitle: row.position_title, labName: row.lab_name, department: row.department, questionAnswers: row.question_answers || {}, piNotes: row.pi_notes ?? null });
}));

router.get('/audit-log', asyncHandler(async (req: Request, res: Response) => {
  const limit = Math.min(parseInt(req.query.limit as string) || 50, 200);
  const offset = parseInt(req.query.offset as string) || 0;
  const result = await pool.query(`SELECT al.* FROM audit_log al WHERE al.admin_id = $1 ORDER BY al.created_at DESC LIMIT $2 OFFSET $3`, [req.userId!, limit, offset]);
  const countResult = await pool.query(`SELECT COUNT(*)::int AS total FROM audit_log WHERE admin_id = $1`, [req.userId!]);
  return res.json({ entries: result.rows.map((row) => ({ id: row.id, action: row.action, resourceType: row.resource_type, resourceId: row.resource_id, ipAddress: row.ip_address, createdAt: row.created_at, metadata: row.metadata })), total: countResult.rows[0]?.total ?? 0, limit, offset });
}));
