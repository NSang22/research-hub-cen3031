...
router.post('/', authMiddleware, requireRole('pi'), asyncHandler(async (req, res) => {
...
router.get('/mine', authMiddleware, requireRole('pi'), asyncHandler(async (req, res) => {
    const piResult = await pool.query('SELECT id FROM pi_profiles WHERE user_id = $1', [req.userId]);
    const pi = piResult.rows[0];
    if (!pi) {
        return res.json([]);
    }
    const result = await pool.query(`SELECT rp.*,
       (SELECT COUNT(*) FROM applications a WHERE a.position_id = rp.id) as app_count
     FROM positions rp
     WHERE rp.pi_id = $1
     ORDER BY rp.created_at DESC`, [pi.id]);
...
router.get('/recommended', authMiddleware, requireRole('student'), asyncHandler(async (req, res) => {
    const studentResult = await pool.query('SELECT id, skills, gpa FROM student_profiles WHERE user_id = $1', [req.userId]);
    const student = studentResult.rows[0];
    if (!student) {
        return res.json([]);
    }
    const studentSkills = student.skills || [];
    const studentGpa = student.gpa != null ? parseFloat(student.gpa) : null;
    const result = await pool.query(`SELECT rp.*, pp.department, pp.lab_name, pp.lab_website
     FROM positions rp
     JOIN pi_profiles pp ON pp.id = rp.pi_id
     WHERE rp.is_open = true
       AND rp.id NOT IN (
    ...
    const scored = result.rows.map((row) => {
        ...
        const minGpa = row.min_gpa != null ? parseFloat(row.min_gpa) : null;
        let skillScore = 0;
        if (requiredSkills.length === 0) {
            skillScore = 1;
        }
        else {
            const matchCount = requiredSkills.filter((s) => studentSkills.some((ss) => ss.toLowerCase() === s.toLowerCase())).length;
...
router.put('/:id', authMiddleware, requireRole('pi'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    const result = await pool.query(`UPDATE positions SET
...
router.delete('/:id', authMiddleware, requireRole('pi'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const piResult = await pool.query('SELECT id FROM pi_profiles WHERE user_id = $1', [req.userId]);
    const pi = piResult.rows[0];
    if (!pi) {
    ...
    try {
        ...
        const result = await client.query(`UPDATE positions SET status = 'closed' WHERE id = $1 AND pi_id = $2 RETURNING id`, [id, pi.id]);
        if (result.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ error: 'Position not found or access denied' });
        }
        await client.query(`UPDATE applications SET status = 'withdrawn'
       WHERE position_id = $1 AND status IN ('pending', 'reviewing')`, [id]);
